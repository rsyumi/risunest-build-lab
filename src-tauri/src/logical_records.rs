...
pub(crate) fn invalid(message: impl Into<String>) -> LogicalRecordError {
...
pub(crate) fn validate_hash(value: &str, description: &str) -> Result<(), LogicalRecordError> {
...
pub(crate) fn validate_object_descriptor(hash: &str, size: u64) -> Result<(), LogicalRecordError> {
...
impl LogicalRecordEnvelope {
    ...
    #[cfg_attr(not(test), allow(dead_code))]
    pub(crate) fn dependency_hashes(&self) -> Vec<String> {
...
pub(crate) fn encoded_object(bytes: Vec<u8>) -> Result<EncodedLogicalObject, LogicalRecordError> {
