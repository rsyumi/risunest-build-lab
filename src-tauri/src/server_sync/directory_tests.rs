...
fn tls_identity(device: &str) -> (String, reqwest::Certificate, std::thread::JoinHandle<()>) {
    ...
    let task = std::thread::spawn(move || {
        ...
        let body = serde_json::json!({"head":{"libraryId":"library","epoch":"epoch","seq":"0","headId":"a".repeat(64),"minRetainedSeq":"0"},"deviceId":device,"operationWatermark":"0","operationPending":false}).to_string();
