...
fn checkpoint_and_each_device_pin_survive_concurrent_commit_and_collection() {
    ...
    let checkpoint = store.create_checkpoint(&b).unwrap();
    let pin = store.pin_changes(&b, &h.epoch, &0.into()).unwrap();
    ...
    let h = store.commit(&a, &intent, &h.etag()).unwrap().head;
    store.acknowledge(&a, &h.epoch, &h.seq).unwrap();
    store.acknowledge(&b, &h.epoch, &h.seq).unwrap();
    ...
    assert_eq!(
        store
            .changes(&h.epoch, &ChangeCursor::after_commit(0.into()), &h.seq, 10)
            .err()
            .unwrap()
            .code,
        "checkpoint-required"
...
fn offline_ack_keeps_tombstones_history_and_staged_objects_and_epoch_rotates() {
    ...
    assert_eq!(store.record("k").unwrap(), c.changes[0].after);
    ...
    assert!(store.record("pending").unwrap() != RecordVersion::Absent);
    assert_eq!(
        store
            .pin_changes(&a, &h.epoch, &0.into())
            .err()
            .unwrap()
            .code,
        "epoch-changed"
