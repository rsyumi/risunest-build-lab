use super::{json, parse, random_id, uploads::now, ChangeCursor, ChangePage, Device, Store};
use crate::{Error, Result};
use risunest_sync_wire::{
    RecordVersion, RemoteHead, Sequence, MAX_METADATA_BYTES, MAX_PAGE_RECORDS,
};
...
impl Store {
    pub fn pin_changes(&self, device: &Device, epoch: &str, after: &Sequence) -> Result<ReadPin> {
        let db = self.db()?;
        ...
        if after < &head.min_retained_seq {
        ...
        let id = random_id()?;
        db.execute(
            "INSERT INTO read_pins VALUES(?1,?2,?3,?4,?5)",
            params![id, device.id, after.as_str(), json(&head)?, now()? + 86400],
        )?;
        Ok(ReadPin {
            pin_id: id,
    ...
    pub fn pinned_changes(
        ...
        let (start, head) = {
            let db = self.reader()?;
            Self::require_device(&db, device)?;
            let (start, body, expires): (String, String, i64) = db
                .query_row(
                    "SELECT after_seq,through,expires FROM read_pins WHERE id=?1 AND device=?2",
                    params![id, device.id],
            ...
            (Sequence::try_from(start)?, parse::<RemoteHead>(&body)?)
        };
        if after.seq < start {
            return Err(Error::new("invalid-cursor", 400));
        }
        self.changes(&head.epoch, after, &head.seq, limit)
    }
    ...
    pub fn create_checkpoint(&self, device: &Device) -> Result<Checkpoint> {
        let mut db = self.db()?;
        Self::require_device(&db, device)?;
        let count: i64 = db.query_row(
        ...
        tx.execute(
            ...
            params![id, device.id, json(&head)?, now()? + 86400],
        )?;
        tx.execute(
            "INSERT INTO checkpoint_records SELECT ?1,key,version FROM records",
            [&id],
        )?;
        tx.commit()?;
        Ok(Checkpoint {
    ...
    pub fn checkpoint_page(
        ...
        Self::require_device(&db, device)?;
        let (body, expires): (String, i64) = db
            .query_row(
                "SELECT head,expires FROM checkpoints WHERE id=?1 AND device=?2",
                params![id, device.id],
        ...
        let mut statement=db.prepare("SELECT key,version FROM checkpoint_records WHERE checkpoint=?1 AND key>?2 ORDER BY key LIMIT ?3")?;
        let mut rows = statement.query(params![id, after.unwrap_or(""), limit as i64 + 1])?;
        ...
        while let Some(row) = rows.next()? {
            ...
            {
                next_key = records.last().map(|r| r.key.clone());
                break;
            }
            used += size;
            records.push(record);
