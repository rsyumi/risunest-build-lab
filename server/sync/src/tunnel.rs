...
async fn run_child(
    ...
    let mut child = command
        .spawn()
        .map_err(|_| Error::new("tunnel-start-failed", 503))?;
    #[cfg(windows)]
    ...
    let outcome = loop {
        tokio::select! {
            ...
            _ = child.wait() => break Err(Error::new("tunnel-exited", 503)),
            line = rx.recv() => match line {
                Some(line) => if let Some(endpoint) = readiness.observe(&line) {
                    if !ready {
                        if let Err(error) = store.observe_tunnel_endpoint(&endpoint) { break Err(error); }
                        ready = true;
                        status.send_replace(TunnelStatus { phase: "connected", endpoint: Some(endpoint), error: None });
                        changed.notify_one();
                    }
                },
