<script lang="ts">
    ...
    let { checked = $bindable(false), label, showLabel = false, disabled = false, id, onchange }: Props = $props()
</script>
...
<label class="inline-flex items-center gap-2 rounded-md text-textcolor focus-within:outline focus-within:outline-2 focus-within:outline-darkborderc focus-within:outline-offset-2 {disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}">
    <input {id} class="sr-only" type="checkbox" bind:checked {disabled} onchange={(event) => onchange?.(event.currentTarget.checked)} />
    <span class="flex h-5 w-5 min-h-5 min-w-5 items-center justify-center rounded-md border-2 border-darkborderc transition-colors duration-200 {checked ? 'bg-darkborderc' : 'bg-darkbutton'}" aria-hidden="true">
        {#if checked}
