...
// Token management loop (If current token usage exceeds allowed amount)
    ...
    await processor.addText(
        data.chunks
            ...
            .map((v) => searchDocumentPrefix + v.text.trim()) // sometimes this should not be used at all. RisuNest does not support embedding model that this is meaningful, isn't it?
    );
