...
const linux = read("src-tauri/tauri.linux.conf.json");
const agent = read("src-tauri/tauri.agent.conf.json");
const bench = read("benchmarks/linux/tauri.conf.json");
...
assert(
  linux.bundle.linux.deb.depends.includes("libc6 (>= 2.39)"),
  "Debian glibc baseline absent",
);
...
assert.match(read("package.json").scripts["linux:build:agent"], /--bundles deb,appimage/);
// The harness is a separate entry: its own identifier, its own frontend, and
// no bundle, so it can never be mistaken for a shipped package.
assert.equal(bench.identifier, "io.github.rsyumi.risunest.linux.bench");
assert.equal(bench.bundle.active, false);
assert.match(bench.build.beforeBuildCommand, /--mode agent/);
const sources = [];
function visit(dir) {
...
console.log("Linux config and agent frontend boundary verified");
