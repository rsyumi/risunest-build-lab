...
use tauri::{
    ...
    Manager, Monitor, PhysicalPosition, PhysicalSize, RunEvent, Runtime, WebviewWindow,
    Window, WindowEvent,
};
...
impl<R: Runtime> AppHandleExt for tauri::AppHandle<R> {
    fn save_window_state(&self, flags: StateFlags) -> Result<()> {
        ...
        let app_dir = plugin_state.directory.clone();
...
impl Builder {
    ...
    /// Sets the directory holding the state file. Defaults to the
    /// identifier-derived application configuration directory.
    pub fn with_directory(mut self, directory: impl Into<PathBuf>) -> Self {
        self.directory.replace(directory.into());
        self
    }
    ...
    pub fn build<R: Runtime>(self) -> TauriPlugin<R> {
        ...
        PluginBuilder::new("window-state")
            ...
            .setup(move |app, _api| {
                ...
                let cache = load_saved_window_states(&directory, &filename).unwrap_or_default();
